...
df = df[df["Status"].astype(str).str.strip() == "通過"].copy()
...
for (zone_raw, theme_raw, topic_raw), group in df.groupby(["Zone", "Theme", "Topic"], dropna=False):
    ...
    group = group.copy()
    print(f"📂 Processing: {zone} / {theme} / {topic}")
    ...
    if "parsed_date" not in group.columns:
        raise KeyError(f"parsed_date 欄位不存在。group columns = {group.columns.tolist()}")
